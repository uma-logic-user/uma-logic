# app_commercial.py
# UMA-Logic Pro - 商用グレード完成版（UI分離版）
# 6つの独立タブ：予想/結果/的中/収支/資金配分/システム状態
...
def check_hit(prediction: dict, result: dict) -> dict:
    ...
    if honmei in [first, second, third]:
        fukusho_payouts = payouts.get("複勝", {})
        payout = fukusho_payouts.get(str(honmei), 0) if isinstance(fukusho_payouts, dict) else 0
...
with tab6:
    ...
    | ワークフロー | スケジュール | 状態確認 |
    |-------------|-------------|---------|
    | 🐎 予想データ取得 | 土日 07:00 JST | [Actionsで確認](https://github.com/ ) |
    | 📊 レース結果取得 | 土日 18:00 JST | [Actionsで確認](https://github.com/ ) |
    | 💹 リアルタイムオッズ | 手動実行 | [Actionsで確認](https://github.com/ ) |
    """)
